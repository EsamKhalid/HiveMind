...
<body class="bg-gray-100 min-h-screen">
    ...
    <div class="xl:w-[50%] lg:w-[70%] max-w-full mx-auto p-6 bg-white shadow-md rounded-lg mt-2 mb-12">
            ...
            <table class="w-full border-collapse border border-gray-300">
                ...
                <tbody>
                    @foreach ($returnRequest->returnItems as $returnItem)
                        <tr>
                            <td class="border p-2">
                                <a href="{{ route('products.show', $returnItem->orderItem->products->id) }}" class="underline">
                                    {{ $returnItem->orderItem->products->product_name }}
                                </a>
                            </td>
                            <td class="border p-2">{{ $returnItem->orderItem->quantity }}</td>
                            <td class="border p-2">£{{ number_format($returnItem->orderItem->products->price * $returnItem->orderItem->quantity, 2) }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
        ...
        <div class="mt-6 flex space-x-4">
            <form action="{{ route('admin.return.approve', $returnRequest->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to approve this request?');">
                @csrf
                @method('PUT')
            ...
            <form action="{{ route('admin.return.deny', $returnRequest->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to deny this request?');">
                @csrf
                @method('PUT')
