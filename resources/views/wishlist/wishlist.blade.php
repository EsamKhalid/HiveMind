...
<body class="bg-stone-100 dark:bg-stone-950 min-h-screen transition-colours duration-1000">
    ...
    <main class="flex justify-center p-5">
        <div class="h-1/2 w-1/2 mt-5 transition-all duration-1000 flex flex-col items-center p-5">
            @if(count($wishlistItems) === 0)
                <div class="flex justify-center items-center h-full">
                    <div class="py-2 px-4 bg-white text-stone-800 dark:text-yellow-200 dark:bg-stone-600 rounded-lg shadow-md h-fit text-center w-[400px] m-5">
                        <p class="">No items in wishlist</p>
                    </div>
                </div>
            @else
                <div class="overflow-y-auto overflow-x-hidden h-[630px] mb-4 rounded-lg transition-all duration-1000 w-full">
                            ...
                            <div class="bg-white rounded-lg flex justify-between bg-white text-stone-800 dark:text-amber dark:bg-stone-800 text-center items-center p-2 mb-2 w-full">
                                ...
                                <div class="flex justify-center items-center p-2 w-1/3">
                                    <form action="{{ route('basket.add') }}" method="post" class="ml-4">
                                        ...
                                        <button class="py-2 a px-4 bg-white text-grey-800 rounded-lg shadow-md dark:text-amber dark:bg-stone-700 dark:hover:underline hover:underline"
                ...
                <div class="flex justify-center mt-4 w-full">
                    <button type="Checkout"
                        class="bg-yellow-400 text-white mt-5 py-4 px-8 rounded-lg shadow-lg hover:underline dark:bg-stone-900 dark:hover:text-amber dark:text-amber w-1/3 font-bold">
                        @csrf
